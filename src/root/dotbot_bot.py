...
class BotSetScreen(Screen):
    ...
    motorWheelDiameterMM = NumericProperty(26.0)
    ...
    def save(self):
        parametersList = [self.profileName, self.motorWheelDiameterMM, self.motorStepsPerRev, self.motorMaxSpeed, self.motorAcceleration]
        # save to settings folder       
        with open(settingsDir + self.profileName + '.bot', 'wb') as output:
    ...
    def load(self, filePath):
        if os.path.isfile(filePath):
            ...
            self.motorWheelDiameterMM = p[1]
            self.motorStepsPerRev = p[2]
            self.motorMaxSpeed = p[3]
            self.motorAcceleration = p[4]
